 ...
 */
const MAX_INPUT_LENGTH: Record<string, number> = {
  food_entry_name: 200,
  search_query: 200,
  saved_meal_name: 100,
 ...
 */
export function sanitizeString(input: string, field: string): string {
...
export function redactSensitive(message: string): string {
  ...
  redacted = redacted.replace(
    /oauth_signature=([^&\s]{10,})/gi,
    "oauth_signature=[REDACTED]"
  );
  redacted = redacted.replace(
    /oauth_consumer_key=([^&\s]{10,})/gi,
