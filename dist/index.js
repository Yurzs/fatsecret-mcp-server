...
import { dateStringToFatSecretInt, fatSecretIntToDateString, todayAsFatSecretInt, VALID_MEALS, } from "./constants.js";
import { sanitizeString, validateDate, validateCredentials, } from "./security.js";
// ─── Server Init ───────────────────────────────────────────────────────────────
...
// ─── Tool: Search Food ─────────────────────────────────────────────────────────
server.registerTool("fatsecret_search_food", {
...
Returns: List of foods with id, name, description (brief nutrition summary).`,
    ...
    try {
        const data = await makeAppRequest("foods.search", {
...
server.registerTool("fatsecret_get_food", {
...
Returns: Food name, servings list with serving_id, serving description, calories, protein, carbs, fat, and available micronutrients.`,
    ...
    try {
        const data = await makeAppRequest("food.get.v4", { food_id });
        ...
        for (const s of servingList) {
            ...
            if (s.fiber)
                lines.push(`- Fiber: ${s.fiber} g`);
            if (s.sugar)
                lines.push(`- Sugar: ${s.sugar} g`);
            if (s.saturated_fat)
                lines.push(`- Saturated Fat: ${s.saturated_fat} g`);
            if (s.sodium)
                lines.push(`- Sodium: ${s.sodium} mg`);
            if (s.potassium)
                lines.push(`- Potassium: ${s.potassium} mg`);
            if (s.cholesterol)
                lines.push(`- Cholesterol: ${s.cholesterol} mg`);
            if (s.calcium)
                lines.push(`- Calcium: ${s.calcium} mg`);
            if (s.iron)
                lines.push(`- Iron: ${s.iron} mg`);
            if (s.vitamin_a)
                lines.push(`- Vitamin A: ${s.vitamin_a} mcg`);
            if (s.vitamin_c)
                lines.push(`- Vitamin C: ${s.vitamin_c} mg`);
...
server.registerTool("fatsecret_create_food_entry", {
...
Returns: Created food entry with nutrition totals.`,
    ...
    try {
        ...
        const data = await makeUserRequest("food_entry.create", {
            food_id,
...
server.registerTool("fatsecret_edit_food_entry", {
...
Returns: Updated entry with new nutrition values.`,
    ...
    try {
        ...
        if (meal)
            params.meal = meal;
        const data = await makeUserRequest("food_entry.edit", params);
        const entry = data?.food_entries?.food_entry;
        const e = Array.isArray(entry) ? entry[0] : entry;
        if (!e) {
...
server.registerTool("fatsecret_delete_food_entry", {
...
Returns: Confirmation of deletion.`,
    ...
    try {
        await makeUserRequest("food_entry.delete", { food_entry_id });
...
server.registerTool("fatsecret_get_food_entries", {
...
Returns: All food entries grouped by meal with calories, protein, carbs, fat per entry and daily totals.`,
    ...
    try {
        const dateInt = date ? dateStringToFatSecretInt(date) : todayAsFatSecretInt();
        const data = await makeUserRequest("food_entries.get.v2", { date: dateInt });
        ...
        for (const meal of ["Breakfast", "Lunch", "Dinner", "Other"]) {
            ...
            for (const e of mealEntries) {
                lines.push(`- **${e.food_entry_name}** (ID: ${e.food_entry_id}) — ` +
...
server.registerTool("fatsecret_get_food_entries_month", {
...
Returns: Daily totals for each logged day in the month.`,
    ...
    try {
        ...
        const data = await makeUserRequest("food_entries.get_month.v2", { date: dateInt });
...
server.registerTool("fatsecret_create_saved_meal", {
...
Returns: Created saved meal with its ID.`,
    ...
    try {
        ...
        const data = await makeUserRequest("saved_meal.create", params);
...
server.registerTool("fatsecret_add_food_to_saved_meal", {
...
Returns: Confirmation with nutrition details.`,
    ...
    try {
        await makeUserRequest("saved_meal_item.add", {
            saved_meal_id,
            food_id,
            food_entry_name,
            serving_id,
...
server.registerTool("fatsecret_get_saved_meals", {
...
Returns: List of saved meals with their IDs, names, and food contents.`,
    ...
    try {
        const data = await makeUserRequest("saved_meals.get.v2");
        ...
        for (const m of mealList) {
            ...
            if (m.saved_meal_description)
                lines.push(`*${m.saved_meal_description}*`);
            if (m.meals)
                lines.push(`Default meal: ${m.meal_type || "other"}`);
            lines.push("");
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
...
server.registerTool("fatsecret_copy_saved_meal_to_diary", {
...
Returns: Confirmation that entries were created.`,
    ...
    try {
        const dateInt = date ? dateStringToFatSecretInt(date) : todayAsFatSecretInt();
        await makeUserRequest("food_entries.copy_saved_meal", {
            saved_meal_id,
            meal,
            date: dateInt,
        });
...
// ─── Tool: Update Weight ───────────────────────────────────────────────────────
server.registerTool("fatsecret_update_weight", {
    title: "Log Weight",
...
Returns: Confirmation of logged weight.`,
    ...
    try {
        const dateInt = date ? dateStringToFatSecretInt(date) : todayAsFatSecretInt();
        await makeUserRequest("weight.update", {
...
server.registerTool("fatsecret_get_weights_month", {
...
Returns: All weight entries for the month with dates.`,
    ...
    try {
        const dateInMonth = new Date(year, month - 1, 1);
        const dateInt = Math.floor(dateInMonth.getTime() / (1000 * 60 * 60 * 24));
        const data = await makeUserRequest("weights.get_month.v2", { date: dateInt });
...
server.registerTool("fatsecret_get_recently_eaten", {
...
Returns: List of recently logged foods with IDs and serving details.`,
    ...
    try {
        const data = await makeUserRequest("foods.get_recently_eaten.v2");
        const foods = data?.foods?.food;
        if (!foods || (Array.isArray(foods) && foods.length === 0)) {
...
server.registerTool("fatsecret_copy_day", {
...
Returns: Confirmation.`,
    ...
    try {
        ...
        await makeUserRequest("food_entries.copy", {
            from_date: fromInt,
            to_date: toInt,
        });
        return {
